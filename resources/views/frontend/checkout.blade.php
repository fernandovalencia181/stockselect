            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
                ...
                <div class="lg:col-span-2 space-y-8" x-show="step === 3" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-x-4">
                    <div class="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
                        ...
                        <div class="space-y-4">
                            <label class="flex items-start p-5 border-2 rounded-2xl cursor-pointer hover:bg-gray-50 transition-all"
                                :class="paymentMethod === 'whatsapp' ? 'border-black bg-gray-50' : 'border-gray-100'">
                                <input type="radio" name="payment_method" value="whatsapp"
                                    x-model="paymentMethod" class="mt-1 text-black focus:ring-black">
                                <div class="ml-4 flex flex-col">
                                    <span class="font-bold text-gray-900">Acordar pago por WhatsApp</span>
                                    <span class="text-xs text-gray-500 mt-1">Bizum, Transferencia o Efectivo — Te contactaremos de inmediato.</span>
                                </div>
                            </label>

                            <label class="flex items-start p-5 border-2 rounded-2xl cursor-pointer hover:bg-gray-50 transition-all"
                                :class="paymentMethod === 'stripe' ? 'border-black bg-gray-50' : 'border-gray-100'">
                                <input type="radio" name="payment_method" value="stripe"
                                    x-model="paymentMethod" class="mt-1 text-black focus:ring-black">
                                <div class="ml-4 flex-1">
                                    <div class="flex items-center justify-between">
                                        <span class="font-bold text-gray-900">Tarjeta (Stripe)</span>
                                        <div class="flex items-center gap-2">
                                            <img src="{{ asset('images/logo-stripe.png') }}" alt="Stripe" class="h-4 w-auto object-contain">
                                            <div class="flex gap-1 h-3 opacity-60">
                                                <img src="{{ asset('images/payments/visa_gris.svg') }}" alt="Visa">
                                                <img src="{{ asset('images/payments/mastercard_gris.svg') }}" alt="Master">
                                            </div>
                                        </div>
                                    </div>
                                    <span class="text-xs text-gray-500 mt-1">Procesamiento seguro. Visa, Mastercard, AMEX.</span>
                                </div>
                            </label>
                        </div>
                        ...
                        <div class="mt-12 bg-gray-50 p-6 rounded-2xl border border-gray-100 relative">
                            ...
                            <button type="button" @click="submitOrder()"
                                ...
                                class="w-full bg-black text-white font-extrabold py-5 rounded-2xl hover:bg-gray-800 transition shadow-xl active:scale-[0.98] flex items-center justify-center gap-3 disabled:opacity-60 disabled:cursor-not-allowed disabled:scale-100">
                                {{-- Estado normal --}}
                                <template x-if="!isSubmitting">
                                    <span class="flex items-center gap-3">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg>
                                        Finalizar Mi Pedido Seguro
                                    </span>
                                </template>
                                {{-- Estado enviando --}}
                                <template x-if="isSubmitting">
